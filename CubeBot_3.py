...
def start_message(message):
    ...
    if chatid == userid:
        ...
        bot.send_message(message.chat.id, "Добро пожаловать, в Cube Bot!\n"
                                          ...
                                          "/bonuslave - бонус\n"
                                          "/advice [текст] - предложение для доработок", reply_markup=markup)
    else:
        bot.send_message(message.chat.id, "Добро пожаловать, в Cube Bot!\n"
                                          "Испытай удачу, бросив кубик 🎲\n"
                                          "Делай ставочки, угадывай числа\n"
...
def advice(message):
    chatid = -443076596
    bot.send_message(chatid, "Oт @%s, %s\n%s\n-------------------------" %
...
def endgame(message):
    ...
    cur.execute("DELETE FROM PREVBETS WHERE UserId = %i AND IDChat = %i" % (userid, chatid))
    conn.commit()
    ...
    for uid in IDs:
        ...
        cur.execute("SELECT Bet, Numbers IDChat FROM PREVBETS WHERE UserId = %i AND IDChat = %i" % (userid, chatid))
        qwe = cur.fetchall()
