...
async def alldataCHAT(chatid, title, date):
...
async def shake(name, userid, chatid, date):
        ...
        try:
            ...
            for k in range(len(plays)):
                ...
                if title != 'Хранитель 🔮':
                    ...
                    await bot.send_animation(chat_id=chatid, animation="CgACAgQAAxkBAALBIV7iUctPBymhH4uP_w1-RcAOXsjjAAITAgACIUyNUsXRudOgGDzCGgQ")
                    await bot.send_message(chatid, f"⭐️ {list_of_names[list_of_plays[i]]} получает достижение "
                    f"\n<b>{title}</b>\n"
                    f"Держи награду +{moni}")
